...
$SUDO docker run -d -it --entrypoint ${ENTRYPOINT} --network ric --ip ${XAPP_IP} \
    -e DBAAS_SERVICE_HOST=$DBAAS_IP -e DBAAS_SERVICE_PORT=$DBAAS_PORT --name ${CONTAINER_NAME} \
    --mount src="/root/xapp-oai/base-xapp",target=/dev_xapp_folder,type=bind \
${IMAGE_NAME}:latest

if [ -n "${GNB_ID}" ]; then
    docker exec ${CONTAINER_NAME} sed -i "s/^export GNB_ID.*/export GNB_ID=${GNB_ID}/g" /home/xapp-oai/xapp-sm-connector/build_xapp.sh
fi

docker exec ${CONTAINER_NAME} sed -i "s/^export XAPP_ID.*/export XAPP_ID=${XAPP_ID}/g" /home/xapp-oai/xapp-sm-connector/build_xapp.shà
# docker exec ${CONTAINER_NAME} /home/xapp-oai/xapp-sm-connector/build_xapp.sh -c
